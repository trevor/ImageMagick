...
static Quantum GenerateNoise(RandomInfo *random_info,const Quantum pixel,
  const NoiseType noise_type,const MagickRealType attenuate)
{
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      if ((channel & RedChannel) != 0)
        q->red=GenerateNoise(random_info[id],p->red,noise_type,attenuate);
      if ((channel & GreenChannel) != 0)
        q->green=GenerateNoise(random_info[id],p->green,noise_type,attenuate);
      if ((channel & BlueChannel) != 0)
        q->blue=GenerateNoise(random_info[id],p->blue,noise_type,attenuate);
      if ((channel & OpacityChannel) != 0)
        q->opacity=GenerateNoise(random_info[id],p->opacity,noise_type,
          attenuate);
      if (((channel & IndexChannel) != 0) &&
          (image->colorspace == CMYKColorspace))
        noise_indexes[x]=(IndexPacket) GenerateNoise(random_info[id],indexes[x],
          noise_type,attenuate);
      p++;
      q++;
    }
  ...
  const MagickEvaluateOperator op,const MagickRealType value)
{
  ...
  switch (op)
  {
    ...
    {
      result=(MagickRealType) GenerateNoise(random_info,pixel,GaussianNoise,
        value);
      break;
    }
    case ImpulseNoiseEvaluateOperator:
    {
      result=(MagickRealType) GenerateNoise(random_info,pixel,ImpulseNoise,
        value);
      break;
    }
    case LaplacianNoiseEvaluateOperator:
    {
      result=(MagickRealType) GenerateNoise(random_info,pixel,LaplacianNoise,
        value);
      break;
    }
    ...
    {
      result=(MagickRealType) GenerateNoise(random_info,pixel,
    ...
    {
      result=(MagickRealType) GenerateNoise(random_info,pixel,PoissonNoise,
        value);
      break;
    }
    ...
    {
      result=(MagickRealType) GenerateNoise(random_info,pixel,UniformNoise,
        value);
      break;
    }
