...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  ReadHALDImage() creates a HALD Clut image and returns it.  It
  ...
  ExceptionInfo *exception)
{
  ...
  */
  ...
  if (level == 0)
    level=8;
  status=MagickTrue;
  image->columns=level*level*level;
  image->rows=level*level*level;
  image_view=AcquireCacheView(image);
#if defined(MAGICKCORE_OPENMP_SUPPORT)
  #pragma omp parallel for schedule(dynamic,4) shared(status)
#endif
  for (y=0; y < (long) image->rows; y+=level)
  {
    long
      blue,
      green,
    ...
    if (status == MagickFalse)
      continue;
    q=QueueCacheViewAuthenticPixels(image_view,0,y,image->columns,level,
      exception);
    ...
    blue=y/level;
    for (green=0; green < (long) (level*level); green++)
    {
      for (red=0; red < (long) (level*level); red++)
      {
        q->red=RoundToQuantum(QuantumRange*red/(level*level-1.0));
        q->green=RoundToQuantum(QuantumRange*green/(level*level-1.0));
        q->blue=RoundToQuantum(QuantumRange*blue/(level*level-1.0));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  RegisterHALDImage() adds attributes for the HALD X image format to the list
...
ModuleExport unsigned long RegisterHALDImage(void)
{
  ...
  entry->description=ConstantString("Identity Hald CLUT image");
