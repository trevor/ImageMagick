  ...
  */
  ...
  do
  {
    ...
    */
    ...
    pixels=AcquireQuantumMemory(length,sizeof(*pixels));
    ...
    switch (image->compression)
    {
      ...
      case RLECompression:
      {
        compress_pixels=AcquireQuantumMemory(length+256,
      ...
      default:
      {
        compress_pixels=AcquireQuantumMemory(length+(length >> 3),
          sizeof(*compress_pixels));
        if (compress_pixels == (unsigned char *) NULL)
          {
            pixels=(unsigned char *) RelinquishMagickMemory(pixels);
        ...
        previous_pixels=AcquireQuantumMemory(length,
