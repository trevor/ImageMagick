    ...
    */
    ...
    for ( ; image; image=image->next)
    {
      ...
      if ((clone == (Image *) NULL) || (exception->severity >= ErrorException))
        break;
      (void) AddValueToSplayTree(magick_registry,clone,clone);
      sv=newSViv((IV) clone);
      rv=newRV(sv);
      av_push(av,sv_bless(rv,hv));
      SvREFCNT_dec(sv);
    ...
    if (difference_image != (Image *) NULL)
      {
        ...
        (void) AddValueToSplayTree(magick_registry,difference_image,
          difference_image);
        sv=newSViv((IV) difference_image);
        rv=newRV(sv);
        av_push(av,sv_bless(rv,hv));
        SvREFCNT_dec(sv);
      }
              ...
              if (image != (Image *) NULL)
                {
                  ...
                  if (image->mask != (Image *) NULL)
                    {
                      (void) AddValueToSplayTree(magick_registry,image->mask,
                        image->mask);
              ...
              if (image != (Image *) NULL)
                {
                  ...
                  if (image->clip_mask != (Image *) NULL)
                    {
                      (void) AddValueToSplayTree(magick_registry,
                        image->clip_mask,image->clip_mask);
  ...
  PPCODE:
  {
    ...
    for ( ; image; image=image->next)
    {
      ...
      if (preview_image == (Image *) NULL)
        goto PerlException;
      (void) AddValueToSplayTree(magick_registry,preview_image,preview_image);
