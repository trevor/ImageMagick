  ...
  const MagickPixelPacket *q,const MagickRealType point,MagickPixelPacket *pixel)
{
  ...
  pixel->green=gamma*Blend_((MagickRealType) p->green,
    (MagickRealType) QuantumRange-(1.0-point)*(QuantumRange-q->opacity),
    (MagickRealType) q->green,(MagickRealType) (QuantumRange-point*(QuantumRange-
    q->opacity)));
...
#define HaldClutImageTag  "Clut/Image"
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      point.x=(MagickRealType) (level-1)*QuantumScale*q->red;
      point.y=(MagickRealType) (level-1)*QuantumScale*q->green;
      point.z=(MagickRealType) (level-1)*QuantumScale*q->blue;
