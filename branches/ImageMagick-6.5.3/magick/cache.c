...
MagickExport Cache AcquirePixelCacheInfo(const unsigned long number_threads)
{
  ...
  cache_info=(CacheInfo *) AcquireCachelineMemory(sizeof(*cache_info));
  ...
  const unsigned long number_threads)
{
  ...
  nexus_info=(NexusInfo **) AcquireCachelineMemory(number_threads*
    sizeof(*nexus_info));
  if (nexus_info == (NexusInfo **) NULL)
    ThrowFatalException(ResourceLimitFatalError,"MemoryAllocationFailed");
  for (i=0; i < (long) number_threads; i++)
  {
    nexus_info[i]=(NexusInfo *) AcquireCachelineMemory(sizeof(**nexus_info));
  ...
  const MagickBooleanType clone,ExceptionInfo *exception)
{
  ...
  if ((time_limit != MagickResourceInfinity) &&
      ((time((time_t *) NULL)-cache_timer) >= (MagickOffsetType) time_limit))
