  ...
  if (composite_image != (Image *) NULL)
    {
      ...
      if (composite_options->compose == DistortCompositeOp)
        (void) SetImageArtifact(composite_image,"compose:args",
          composite_options->distort_geometry);
...
static void RelinquishCompositeOptions(CompositeOptions *composite_options)
{
  ...
  if (composite_options->distort_geometry != (char *) NULL)
    composite_options->distort_geometry=(char *)
      RelinquishMagickMemory(composite_options->distort_geometry);
    ...
    switch (*(option+1))
    {
      ...
      case 'd':
      {
        ...
        if (LocaleCompare("distort",option+1) == 0)
          {
            (void) CloneString(&composite_options.distort_geometry,
              (char *) NULL);
            if (*option == '+')
              break;
            i++;
            ...
            if (IsGeometry(argv[i]) == MagickFalse)
              ThrowCompositeInvalidArgumentException(option,argv[i]);
            (void) CloneString(&composite_options.distort_geometry,argv[i]);
