...
static Image *ReadHRZImage(const ImageInfo *image_info,ExceptionInfo *exception)
{
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    {
      q->red=ScaleCharToQuantum(*p++) << 2;
      q->green=ScaleCharToQuantum(*p++) << 2;
...
static MagickBooleanType WriteHRZImage(const ImageInfo *image_info,Image *image)
{
  ...
  */
  ...
  if (hrz_image->colorspace != RGBColorspace)
    (void) SetImageColorspace(hrz_image,RGBColorspace);
  /*
  ...
  */
  for (y=0; y < (long) hrz_image->rows; y++)
  {
    ...
    {
      *q++=ScaleQuantumToChar(p->red) >> 2;
      *q++=ScaleQuantumToChar(p->green) >> 2;
      *q++=ScaleQuantumToChar(p->blue) >> 2;
      p++;
    }
