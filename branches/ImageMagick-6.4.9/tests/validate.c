    ...
    for (j=0; reference_storage[j].type != UndefinedPixel; j++)
    {
      ...
      */
      ...
      if (LocaleNCompare(reference_map[i],"cmy",3) == 0)
        (void) SetImageColorspace(reference_image,CMYKColorspace);
      ...
      */
      ...
      reconstruct_image->colorspace=reference_image->colorspace;
  ...
  else
    {
      ...
      if (status == MagickFalse)
      ...
      else
        {
          ...
          (void) fprintf(stdout,"Copyright: %s\n\n",
            GetMagickCopyright());
          (void) fprintf(stdout,"ImageMagick Validation Suite\n\n");
