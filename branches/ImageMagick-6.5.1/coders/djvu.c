...
get_page_image(LoadContext *lc, ddjvu_page_t *page, int x, int y, int w, int h, QuantumInfo* quantum_info)
{
  int
    ret;
        ...
        unsigned char* q = (unsigned char *) malloc(lc->image->rows *  stride);
...
get_page_line(LoadContext *lc, int row, QuantumInfo* quantum_info)
{
        ...
        unsigned char* q = (unsigned char *) malloc(lc->image->columns
#if RGB
                         * 3
#endif
                );
  ...
  ExceptionInfo *exception)
{
  ...
  */
  ...
  int use_cache = 0;
  const char* url = "http://maruska.dyndns.org/fake.djvu";
