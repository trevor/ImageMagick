    ...
    *settings[]=
    {
      ...
      "                     maximum RMSE for subimage match",
        ...
        if (LocaleCompare("dissimilarity-threshold",option+1) == 0)
          {
            ...
            if (*option == '+')
              ...
              dissimilarity_threshold=QuantumScale*
                StringToDouble(argv[i],QuantumRange);
            break;
          }
        ThrowCompareException(OptionError,"UnrecognizedOption",option)
      }
      ...
      */
      ...
      if (similarity_image == (Image *) NULL)
        ...
        if (similarity_metric > dissimilarity_threshold)
          {
            ThrowCompareException(ImageError,"ImagesTooDissimilar",
              image->filename);
          }
        else
          {
