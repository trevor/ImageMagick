...
%  closely resembles the template.
%
%  The format of the ExtractSubimageFromImageImage method is:
%
%      Image *ExtractSubimageFromImage(const Image *image,const Image *template,
%        ExceptionInfo *exception)
%
%  A description of each parameter follows:
%
%    o image: the image.
%
%    o template: find an area of the image that closely resembles this image.
%
%    o exception: return any errors or warnings in this structure.
%
*/

static double GetSimilarityMetric(const Image *image,const Image *template,
  ...
  ExceptionInfo *exception)
{
  ...
  */
  similarity=0.0;
  channels=3;
  if ((image->matte != MagickFalse) && (template->matte != MagickFalse))
    channels++;
  if ((image->colorspace == CMYKColorspace) &&
      (template->colorspace == CMYKColorspace))
    channels++;
  image_view=AcquireCacheView(image);
  template_view=AcquireCacheView(template);
  for (y=0; y < (long) template->rows; y++)
  {
    register const IndexPacket
      *indexes,
      *template_indexes;
    ...
    p=GetCacheViewVirtualPixels(image_view,x_offset,y_offset+y,
      template->columns,1,exception);
    q=GetCacheViewVirtualPixels(template_view,0,y,template->columns,1,
      exception);
    if ((p == (const PixelPacket *) NULL) || (q == (const PixelPacket *) NULL))
      continue;
    indexes=GetCacheViewVirtualIndexQueue(image_view);
    template_indexes=GetCacheViewVirtualIndexQueue(template_view);
    for (x=0; x < (long) template->columns; x++)
    {
      MagickRealType
        pixel;
      ...
      if ((image->colorspace == CMYKColorspace) &&
          (template->colorspace == CMYKColorspace))
        {
          pixel=QuantumScale*(indexes[x]-(double) template_indexes[x]);
          similarity+=pixel*pixel;
        }
      p++;
      q++;
    }
    normalized_similarity=sqrt(similarity)/template->columns/template->rows/
      channels;
    if (normalized_similarity > similarity_threshold)
      break;
  }
  template_view=DestroyCacheView(template_view);
  image_view=DestroyCacheView(image_view);
  return(normalized_similarity);
}

MagickExport Image *ExtractSubimageFromImage(Image *image,const Image *template,
  ExceptionInfo *exception)
{
  long
    y;
  ...
  */
  if ((template->columns > image->columns) || (template->rows > image->rows))
    return((Image *) NULL);
  similarity_threshold=HUGE_VAL;
  SetGeometry(template,&offset);
#if defined(MAGICKCORE_OPENMP_SUPPORT)
  #pragma omp parallel for schedule(dynamic,4)
#endif
  for (y=0; y < (long) (image->rows-template->rows); y++)
  {
    double
      similarity;
    ...
    for (x=0; x < (long) (image->columns-template->columns); x++)
    {
      similarity=GetSimilarityMetric(image,template,x,y,similarity_threshold,
        exception);
#if defined(MAGICKCORE_OPENMP_SUPPORT)
  ...
  if (similarity_threshold > (QuantumScale*template->fuzz/100.0))
    return((Image *) NULL);
