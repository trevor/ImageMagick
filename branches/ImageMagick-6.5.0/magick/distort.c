  ...
  max.y = MagickMax(max.y,p.y); \
    ...
    switch (method)
    {
      ...
      case DePolarDistortion:
      {
        ...
        geometry.width = (unsigned long) ceil((coeff[0]+coeff[1])*MagickPI);
        break;
      }
      case ShepardsDistortion:
      case BilinearDistortion:
    ...
    if ( method == DePolarDistortion ) {
      ...
      coeff[7]=(coeff[0]-coeff[1])/geometry.height; /* should be 1.0 */
    }
  }
    ...
    switch (method) {
      ...
      case ArcDistortion:
      {
        ...
        fprintf(stderr, "       xx=xx*%lg %+lg;\n",
                            coeff[1], coeff[4]);
        fprintf(stderr, "       yy=(%lg - hypot(ii,jj)) * %lg;\n",
                            coeff[2], coeff[3]);
      ...
      case PolarDistortion:
      {
        ...
        fprintf(stderr, "       xx=(atan2(ii,jj)%+lg)/(2*pi);\n",
                         -(coeff[4]+coeff[5])/2 );
        fprintf(stderr, "       xx=xx-round(xx);\n");
        fprintf(stderr, "       xx=xx*2*pi*%lg + v.w/2;\n",
                         coeff[6] );
      ...
      case DePolarDistortion:
      {
        ...
        fprintf(stderr, "%s", image_gen);
        fprintf(stderr, "  -fx 'aa=(i+.5)*%lg %+lg;\n", coeff[6], -coeff[4] );
        fprintf(stderr, "       rr=(j+.5)*%lg %+lg;\n", coeff[7], -coeff[1] );
        fprintf(stderr, "       xx=rr*sin(aa) %+lg;\n", coeff[2] );
        fprintf(stderr, "       yy=rr*cos(aa) %+lg;\n", coeff[3] );
        fprintf(stderr, "       v.p{xx-.5,yy-.5}'\n");
        break;
      }
      case BarrelDistortion:
